import { useProjectById } from "@/store/projects/project.selectors";

const ProjectCard = ({ projectId, onClick }: { projectId: string; onClick: () => void }) => {
  const project = useProjectById(projectId);
  const { title, description, statuses } = project || {};
  ...
  return (
    ...
    >
      ...
      <p className="text-xs font-bold mt-auto">Status count: {statuses?.length}</p>
    </div>
  );
};
